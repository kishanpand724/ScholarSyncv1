...
export const CombinationDetailModal: React.FC<CombinationDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs overflow-y-auto">
      <div
        className="relative w-full max-w-2xl bg-white rounded-none shadow-xl border border-[#E5E7EB] overflow-hidden my-8 max-h-[90vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="p-6 sm:p-7 border-b border-[#F3F4F6] flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-mono font-medium text-[#4B5563] bg-[#F3F4F6] px-2 py-0.5 rounded-none">
                AY 2026-27
              </span>
            </div>
        ...
        <div className="p-6 sm:p-7 overflow-y-auto space-y-6">
          {/* Benefit Box */}
          <div className="p-6 rounded-none bg-emerald-50 border border-emerald-200 flex items-center justify-between">
            <div>
              <span className="text-xs font-semibold text-emerald-900 uppercase tracking-wider block">
                Total Potential Combined Benefit
              </span>
              <div className="text-3xl font-bold text-[#111827] mt-1.5">
          ...
          <div className="p-5 rounded-none bg-[#F9FAFB] border border-[#E5E7EB] space-y-3">
          ...
          <div className="space-y-3.5">
            ...
            <div className="space-y-3">
              {combination.scholarships.map((s, idx) => (
                <div
                  key={s.id}
                  className="p-4.5 rounded-none bg-[#F9FAFB] border border-[#F3F4F6] flex items-center justify-between gap-4"
                >
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-none bg-white text-[#111827] border border-[#E5E7EB]">
                        Scheme #{idx + 1}
                      </span>
                      <span className="text-xs text-[#6B7280]">
                        {s.classification}
                      ...
                      {s.scheme_type === 'merit_based' ? (
                        <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-[#1D4ED8] bg-[#EFF6FF] px-2 py-0.5 rounded-none">
                          <Award className="w-2.5 h-2.5" />
                          <span>Merit</span>
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-[#6D28D9] bg-[#F5F3FF] px-2 py-0.5 rounded-none">
                  ...
                  <div className="text-right shrink-0 flex flex-col items-end gap-2">
                    ...
                    <button
                      ...
                      className="text-xs font-semibold px-3 py-1.5 rounded-none bg-white border border-[#E5E7EB] text-[#374151] hover:text-[#111827] hover:border-[#D1D5DB] transition-all cursor-pointer"
                    >
                      Details
                    </button>
                  </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {combination.merged_documents.map((doc, idx) => (
                <div
                  key={idx}
                  className="p-3 rounded-none bg-[#F9FAFB] border border-[#F3F4F6] text-xs text-[#374151] flex items-center gap-2.5"
                >
        ...
        {/* Modal Footer */}
        <div className="p-5 sm:p-6 bg-[#F9FAFB] border-t border-[#F3F4F6] flex items-center justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-6 py-2.5 rounded-none bg-emerald-800 hover:bg-emerald-900 text-white text-xs font-bold transition-all shadow-xs cursor-pointer"
          >
            Done
          </button>
        </div>
