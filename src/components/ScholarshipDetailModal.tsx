...
export const ScholarshipDetailModal: React.FC<ScholarshipDetailModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-4xl bg-white rounded-none shadow-2xl border border-[#E5E7EB] overflow-hidden my-6 max-h-[92vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* HEADER */}
        <div className="p-6 sm:p-7 border-b border-[#F3F4F6] bg-white sticky top-0 z-10">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-2 flex-1 min-w-0">
              {/* Badges */}
              <div className="flex flex-wrap items-center gap-2">
                {scholarship.source_type === 'MAHADBT' || scholarship.id.startsWith('MAHADBT') ? (
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-none bg-emerald-50 text-emerald-800 border border-emerald-300">
                    MahaDBT (Govt of Maharashtra)
                  </span>
                ) : (
                  <span className="text-xs font-bold px-2.5 py-0.5 rounded-none bg-blue-50 text-blue-800 border border-blue-300">
                    NSP (Govt of India)
                  </span>
                )}
                ...
                {scholarship.department && (
                  <span className="text-xs font-medium px-2 py-0.5 rounded-none bg-amber-50 text-amber-800 border border-amber-200">
                    {scholarship.department}
                  </span>
                )}
                <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-none bg-[#F3F4F6] text-[#111827]">
                  AY {scholarship.academic_year || '2026-27'}
                </span>
                {scholarship.scheme_type === 'merit_based' ? (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-none bg-[#EFF6FF] text-[#1D4ED8] border border-[#DBEAFE]">
                    <Award className="w-3.5 h-3.5" />
                    <span>Merit-based Scheme</span>
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1 text-xs font-semibold px-2.5 py-0.5 rounded-none bg-[#F5F3FF] text-[#6D28D9] border border-[#EDE9FE]">
                    <HeartHandshake className="w-3.5 h-3.5" />
                    <span>Welfare-based Scheme</span>
                  </span>
                )}
                {validationStatus === 'verified' && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-semibold px-2 py-0.5 rounded-none bg-emerald-50 text-emerald-800 border border-emerald-200">
            ...
            <button
              onClick={onClose}
              className="p-2 rounded-none text-[#6B7280] hover:text-[#111827] hover:bg-[#F3F4F6] transition-colors cursor-pointer shrink-0"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
          ...
          <div className="flex items-center gap-2 mt-5 pt-4 border-t border-[#F3F4F6]">
            <button
              ...
              className={`px-3.5 py-2 text-xs font-semibold rounded-none transition-colors cursor-pointer ${
                activeTab === 'overview'
            ...
            {matchResult && (
              <button
                ...
                className={`px-3.5 py-2 text-xs font-semibold rounded-none transition-colors flex items-center gap-1.5 cursor-pointer ${
              ...
              >
                ...
                {matchResult.status === 'not_eligible' && (
                  <span className="w-2 h-2 rounded-none bg-rose-400"></span>
                )}
              </button>
            )}
            <button
        ...
        <div className="p-6 sm:p-7 overflow-y-auto space-y-7 flex-1">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-7">
              ...
              <div className="p-6 rounded-none bg-emerald-50 border border-emerald-200 flex flex-col sm:flex-row sm:items-center justify-between gap-5">
                <div>
                  <span className="text-xs font-semibold text-emerald-900 uppercase tracking-wider block">
                    Potential Financial Benefit
                  </span>
                  <div className="text-3xl font-extrabold text-[#111827] mt-1">
                ...
                <div className="sm:text-right pt-3 sm:pt-0 border-t sm:border-t-0 border-emerald-200">
              ...
              {scholarship.description && (
                <div className="p-5 rounded-none bg-gray-50 border border-gray-200">
              ...
              <div className="rounded-none border border-gray-200 p-6 bg-white space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  {elig.family_conditions && elig.family_conditions.length > 0 && (
                    <div className="p-4 rounded-none bg-amber-50/60 border border-amber-100">
                      <span className="font-bold text-amber-900 block mb-1.5">Family & Children Constraints:</span>
                  ...
                  {elig.institution_requirements && elig.institution_requirements.length > 0 && (
                    <div className="p-4 rounded-none bg-blue-50/60 border border-blue-100">
                      <span className="font-bold text-blue-900 block mb-1.5">Institution Accreditation Requirements:</span>
                  ...
                  {elig.admission_type && (
                    <div className="p-4 rounded-none bg-purple-50/60 border border-purple-100">
                      <span className="font-bold text-purple-900 block mb-1">Admission Type:</span>
                  ...
                  {elig.special_category_requirements && elig.special_category_requirements.length > 0 && (
                    <div className="p-4 rounded-none bg-rose-50/60 border border-rose-100">
                      <span className="font-bold text-rose-900 block mb-1.5">Special Wards / Beneficiary Quotas:</span>
              ...
              <div className="rounded-none border border-gray-200 p-6 bg-white space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div className="p-3.5 rounded-none bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-900 font-medium block">Tuition / Course Fee</span>
                    ...
                    <div className="p-3.5 rounded-none bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-900 font-medium block">Maintenance Allowance</span>
                      <span className="font-bold text-gray-900 mt-1 block">
                    ...
                    <div className="p-3.5 rounded-none bg-emerald-50 border border-emerald-100">
                      <span className="text-emerald-900 font-medium block">Books / Equipment Aid</span>
                      <span className="font-bold text-gray-900 mt-1 block">
                  ...
                  {ben.components && ben.components.length > 0 && (
                    <div className="mt-4 space-y-2">
                      ...
                      {ben.components.map((comp: any, i: number) => (
                        <div key={i} className="p-3 rounded-none bg-gray-50 border border-gray-200 flex justify-between items-center">
                          <div>
              ...
              <div className="rounded-none border border-gray-200 p-6 bg-white space-y-5">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gray-100 pb-4">
                  ...
                  {(scholarship.source?.source_url || scholarship.source_url) && (
                    <a
                      ...
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-none bg-rose-50 text-rose-700 hover:bg-rose-100 text-xs font-semibold border border-rose-200 transition-colors self-start sm:self-auto"
                ...
                <div className="p-4 rounded-none bg-blue-50/50 border border-blue-100 text-xs text-blue-900 leading-relaxed">
                  <span className="font-bold block mb-1">Scheme Document Requirements:</span>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-4">
                      <div className="flex items-center justify-between text-xs bg-gray-50 p-3 rounded-none border border-gray-200">
                        <span className="font-semibold text-gray-700">Document Readiness Status:</span>
                        <span className={`font-bold px-2.5 py-0.5 rounded-none text-xs ${crossRef.isFullyReady ? 'bg-emerald-100 text-emerald-900' : 'bg-amber-100 text-amber-900'}`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                        {crossRef.checks.map((chk, i) => (
                          <div
                            key={i}
                            className={`flex items-start justify-between gap-3 p-3.5 rounded-none border transition-colors ${
                              chk.isPossessed
                          ...
                          >
                            <div className="flex items-start gap-2.5 min-w-0">
                              <div className={`w-5 h-5 rounded-none flex items-center justify-center shrink-0 mt-0.5 font-bold text-[10px] ${
                                chk.isPossessed ? 'bg-emerald-800 text-white' : 'bg-gray-200 text-gray-700'
                              }`}>
                                {chk.isPossessed ? '✓' : i + 1}
                              </div>
                              <div>
                            ...
                            <div className="shrink-0">
                              {chk.isPossessed ? (
                                <span className="text-[10px] font-bold text-emerald-800 bg-emerald-100 px-2 py-0.5 rounded-none">
                                  In Hand
                                </span>
                              ) : onToggleDocument ? (
                                <button
                                  ...
                                  className="text-[10px] font-semibold text-blue-700 hover:text-blue-900 bg-white hover:bg-blue-50 px-2.5 py-1 rounded-none border border-blue-200 cursor-pointer transition-colors"
                                >
                                  + Mark Possessed
                                </button>
                              ) : (
                                <span className="text-[10px] font-medium text-amber-700 bg-amber-50 px-2 py-0.5 rounded-none border border-amber-200">
                                  Pending
                                </span>
                              )}
                            </div>
                ...
                {doc.submission_format && (
                  <div className="pt-3 flex flex-wrap gap-4 text-gray-500 text-[11px] border-t border-gray-100">
              ...
              <div className="rounded-none border border-gray-200 p-6 bg-white space-y-4">
                ...
                <div className="space-y-3 text-xs">
                  <div className="p-4 rounded-none bg-rose-50/60 border border-rose-100">
                    <span className="font-bold text-rose-900 block mb-1.5">Who CANNOT Apply (Exclusions):</span>
                    <ul className="list-disc list-inside space-y-1 text-rose-900/90">
                  ...
                  <div className="p-4 rounded-none bg-amber-50/60 border border-amber-100">
                    <span className="font-bold text-amber-900 block mb-1.5">Simultaneous Scholarship & Stacking Rules:</span>
          ...
          {activeTab === 'evaluation' && matchResult && (
            <div className="space-y-6">
              ...
              <div
                className={`p-6 rounded-none border flex items-center justify-between gap-5 ${
                  matchResult.status === 'eligible'
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {matchResult.criteria_checks.map((check, i) => (
                    <div
                      key={i}
                      className={`p-4 rounded-none border flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs transition-all ${
                    ...
                    >
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div>
                          ...
                          <p className="text-gray-700 mt-1.5 leading-relaxed">{check.reason}</p>
                        </div>
                      </div>

                      <div className="text-left sm:text-right shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 border-gray-200/60 pl-7 sm:pl-0">
          ...
          {activeTab === 'source' && (
            <div className="space-y-6">
              <div className="p-5 rounded-none bg-blue-50/60 border border-blue-200 text-xs text-blue-900 leading-relaxed">
              ...
              <div className="rounded-none border border-gray-200 p-6 bg-white space-y-5">
                <h4 className="text-xs font-bold text-gray-700 uppercase tracking-wider border-b pb-3">
                  Official Documents & Links
                </h4>

                <div className="space-y-3.5">
                  <div className="p-4 rounded-none bg-gray-50 border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                    <div>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-emerald-800 text-white font-semibold rounded-none hover:bg-emerald-900 transition-colors shrink-0 cursor-pointer"
                    >
                  ...
                  {officialSpecUrl && (
                    <div className="p-4 rounded-none bg-gray-50 border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-[#111827] text-white font-semibold rounded-none hover:bg-black transition-colors shrink-0 cursor-pointer"
                      >
                  ...
                  {officialFaqUrl && (
                    <div className="p-4 rounded-none bg-gray-50 border border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3.5 py-2 bg-gray-200 text-gray-800 font-semibold rounded-none hover:bg-gray-300 transition-colors shrink-0 cursor-pointer"
                      >
                ...
                <div className="pt-3 border-t border-gray-100 text-[11px] text-gray-500 flex flex-wrap justify-between items-center gap-3">
                  <span>Source Validation Status: <strong className="text-emerald-800 capitalize">{validationStatus}</strong></span>
        ...
        {/* MODAL FOOTER */}
        <div className="p-5 sm:p-6 border-t border-[#F3F4F6] bg-gray-50 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 flex-wrap justify-end">
            {scholarship.official_specification_url && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 bg-rose-50 text-rose-800 border border-rose-200 rounded-none hover:bg-rose-100 transition-colors"
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-xs font-semibold px-3.5 py-2 bg-white text-gray-700 border border-gray-300 rounded-none hover:bg-gray-50 transition-colors"
            >
            ...
            {matchResult?.status === 'eligible' && (
              officialApplicationUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-xs font-bold px-4.5 py-2 bg-emerald-800 text-white rounded-none hover:bg-emerald-900 transition-colors shadow-2xs"
                ...
                <span className="inline-flex items-center px-3.5 py-2 rounded-none text-xs font-medium text-[#6B7280] bg-[#F3F4F6] border border-[#E5E7EB]">
            ...
            <button
              onClick={onClose}
              className="px-4.5 py-2 bg-[#111827] text-white text-xs font-bold rounded-none hover:bg-black transition-colors cursor-pointer"
            >
              Close
            </button>
          </div>
